...
public class MSharpUiCleaner : CodeCleanerCommandRunnerBase
{
    public override async Task<SyntaxNode> CleanUpAsync(SyntaxNode initialSourceNode)
    {
        if (ProjectItemDetails.Document.Project.Name == "#UI")
    ...
    private async Task<SyntaxNode> ChangeMethodHelperAsync(SyntaxNode initialSourceNode)
    {
        ...
        var fullSearchRewriter = new FullSearchRewriter(ProjectItemDetails.SemanticModel
            , ProjectItemDetails.Document.Project.Solution
            , ProjectItemDetails.Document, IsReportOnlyMode, Options);
    ...
    private class CombinedExpressionsFormRewriter : CleanupCSharpSyntaxRewriter
    {
        ...
        public override SyntaxNode VisitConstructorDeclaration(ConstructorDeclarationSyntax node)
        {
            var t = node.DescendantNodesOfType<ExpressionStatementSyntax>()
                .Select((r, i) => new StatementType
                {
                    Row = r,
                    Index = i,
                    MethodName = (CSharpExtensions.GetSymbolInfo(_semanticModel, r.Expression).Symbol as IMethodSymbol)?.Name
                });
    ...
    private class IfWorkFlowRewriter : CleanupCSharpSyntaxRewriter
    {
        private SemanticModel _semanticModel;
    ...
    private class CustomColumnRewriter : CleanupCSharpSyntaxRewriter
    {
        private SemanticModel _semanticModel;
        ...
        public override SyntaxNode VisitInvocationExpression(InvocationExpressionSyntax node)
        {
            var customColumnNode = node.DescendantNodesAndSelfOfType<InvocationExpressionSyntax>()
                .Where(x => (CSharpExtensions.GetSymbolInfo(_semanticModel, x).Symbol as IMethodSymbol)?.Name == "CustomColumn").FirstOrDefault();
    ...
    private class SendItemIdRewriter : CleanupCSharpSyntaxRewriter
    {
        private SemanticModel _semanticModel;
    ...
    // column.search.field.
    private class ElementsNewSyntaxRewriter : CleanupCSharpSyntaxRewriter
    {
        private SemanticModel _semanticModel;
    ...
    // cancel/save/delete
    private class FormModuleWorkFlowRewriter : CleanupCSharpSyntaxRewriter
    {
        private SemanticModel _semanticModel;
        ...
        public override SyntaxNode VisitInvocationExpression(InvocationExpressionSyntax node)
        {
            var s = node.DescendantNodesAndSelfOfType<InvocationExpressionSyntax>()
                .Where(x => (CSharpExtensions.GetSymbolInfo(_semanticModel, x).Symbol as IMethodSymbol)?.Name == "OnClick").FirstOrDefault();

            if (s == null)
                return base.VisitInvocationExpression(node);

            var methodSymbol = (CSharpExtensions.GetSymbolInfo(_semanticModel, s).Symbol as IMethodSymbol);
    ...
    // onlick-go -> go
    private class GeneralWorkFlowRewriter : CleanupCSharpSyntaxRewriter
    {
        private SemanticModel _semanticModel;
    ...
    private class ListModuleWorkFlowRewriter : CleanupCSharpSyntaxRewriter
    {
        private SemanticModel _semanticModel;
        ...
        {
            var s = node.DescendantNodesAndSelfOfType<InvocationExpressionSyntax>()
                .Where(x => ((CSharpExtensions.GetSymbolInfo(_semanticModel, x).Symbol as IMethodSymbol)?.Name == "Go" &&
                             x.FirstAncestorOrSelf<ArgumentSyntax>() == null) ||
                            (CSharpExtensions.GetSymbolInfo(_semanticModel, x).Symbol as IMethodSymbol)?.Name == "OnClick" ||
                            (CSharpExtensions.GetSymbolInfo(_semanticModel, x).Symbol as IMethodSymbol)?.Name == "Link").FirstOrDefault();
    ...
    private class FullSearchRewriter : CleanupCSharpSyntaxRewriter
    {
        private SemanticModel _semanticModel;
        private Solution _solution;
        private RoslynDocument _roslynDocument;

        private bool _shouldConvertToFullSearch = false;
            ...
            base(isReportOnlyMode, options)
        {
            ...
            _solution = solution;
            _roslynDocument = roslynDocument;
        }

        public override SyntaxNode VisitVariableDeclaration(VariableDeclarationSyntax node)
        {
            var declarationSymbol = CSharpExtensions.GetSymbolInfo(_semanticModel, node.Type).Symbol;
            var identifierName = CSharpExtensions.GetDeclaredSymbol(_semanticModel, node.Variables.FirstOrDefault());
                        ...
                        .Initializer.Value.As<InvocationExpressionSyntax>().MethodNameShouldBe("Icon"))
                {
                    ...
                    if (result.Count() == 1 && invocation.ArgumentsCountShouldBe(0))
                    {
                        ...
                        if (nextNode.Ancestors().OfType<InvocationExpressionSyntax>().Any())
                        {
                            ...
                            if (isNextInvocOk)
                            {
                                _shouldConvertToFullSearch = true;
        ...
        {
            var s = node.DescendantNodesAndSelfOfType<InvocationExpressionSyntax>()
                .Where(x => (CSharpExtensions.GetSymbolInfo(_semanticModel, x).Symbol as IMethodSymbol)?.Name == "AfterControlAddon").FirstOrDefault();
        ...
        public Task<IEnumerable<ReferencedSymbol>> GetReferencedSymbolsAsync(ISymbol symbol)
        {
            return SymbolFinder.FindReferencesAsync(symbol,
                _solution, documents:
                ImmutableHashSet.Create(_roslynDocument));
        }
        ...
        public override SyntaxNode VisitInvocationExpression(InvocationExpressionSyntax node)
        {
            ...
            if (s != null && _shouldConvertToFullSearch)
            {
                _shouldConvertToFullSearch = false;
    ...
    private class CustomFieldRewriter : CleanupCSharpSyntaxRewriter
    {
        private SemanticModel _semanticModel;
        ...
        {
            var customColumnNode = node.DescendantNodesAndSelfOfType<InvocationExpressionSyntax>()
                .Where(x => (CSharpExtensions.GetSymbolInfo(_semanticModel, x).Symbol as IMethodSymbol)?.Name == "CustomField"
    ...
    private class OnClickGoWorkFlowRewriter : CleanupCSharpSyntaxRewriter
    {
        private SemanticModel _semanticModel;
        ...
        public override SyntaxNode VisitInvocationExpression(InvocationExpressionSyntax node)
        {
            ...
            var invocation = node.DescendantNodesOfType<InvocationExpressionSyntax>()
                .Where(x =>
                    //x.Expression.IsKind(SyntaxKind.SimpleMemberAccessExpression) &&
                    (
                        (CSharpExtensions.GetSymbolInfo(_semanticModel, x).Symbol as IMethodSymbol) != null &&
                        (CSharpExtensions.GetSymbolInfo(_semanticModel, x).Symbol as IMethodSymbol).ReturnsVoid &&
                        (CSharpExtensions.GetSymbolInfo(_semanticModel, x).Symbol as IMethodSymbol).Name == "Go")
    ...
    private class MergedUpRewriter : CleanupCSharpSyntaxRewriter
    {
        private SemanticModel _semanticModel;
        public override SyntaxNode VisitClassDeclaration(ClassDeclarationSyntax node)
        {
