...
public static class Extensions
{
    ...
    {
        if (text != null && text.StartsWith(textToTrim, ignoreCase, System.Globalization.CultureInfo.InvariantCulture))
        {
    ...
    /// </summary>
    public static TK Get<T, TK>(this T item, Func<T, TK> selector)
    {
        if (ReferenceEquals(item, null))
            return default(TK);
        return (selector != null) ? selector(item) : default(TK);
        //else
        //{
        //    try
        //    {
    ...
    private const string SingleQuote = "'", DoubleQuote = "\"";
