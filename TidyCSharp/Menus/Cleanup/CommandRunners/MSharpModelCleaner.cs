...
namespace Geeks.VSIX.TidyCSharp.Cleanup
{
	public class MSharpModelCleaner : CodeCleanerCommandRunnerBase, ICodeCleaner
	{
		...
		SyntaxNode ChangeMethodHelper(SyntaxNode initialSourceNode)
		{
			...
			if (IsReportOnlyMode)
			{
				this.CollectMessages(localTimeRewriter.GetReport());
				this.CollectMessages(cascadeDeleteRewriter.GetReport());
				this.CollectMessages(calculatedGetterRewriter.GetReport());
				this.CollectMessages(transientDatabaseModeRewriter.GetReport());
				return initialSourceNode;
			}
